...
class DetectorTest {
    ...
    @Test
    fun detectionRequiresThreeConsecutiveFrames() {
        detectorFixture(classCount = 1).use { fixture ->
            val detection = endToEnd(10f, 10f, 30f, 30f, 0.95f, 0f)
            fixture.process(detection, 1_000L)
            fixture.process(detection, 1_100L)
            assertTrue(fixture.detector.detections.value.isEmpty())

            fixture.process(endToEnd(), 1_200L)
            fixture.process(detection, 1_300L)
            assertTrue(fixture.detector.detections.value.isEmpty())

            fixture.process(detection, 1_400L)
            fixture.process(detection, 1_500L)
            assertEquals(1, fixture.detector.detections.value.size)
        }
    }

    @Test
    fun boundingBoxesAreAveragedAcrossFourFrames() {
        detectorFixture(classCount = 1).use { fixture ->
            ...
            assertEquals(25f, box.left, 0.001f)
            assertEquals(35f, box.right, 0.001f)
    ...
    fun notificationUsesStrictSixSecondCooldown() {
        detectorFixture(classCount = 1).use { fixture ->
            val output = endToEnd(10f, 10f, 30f, 30f, 0.95f, 0f)
            fixture.process(output, 800L)
            fixture.process(output, 900L)
    ...
    private fun detectorFixture(classCount: Int): DetectorFixture {
