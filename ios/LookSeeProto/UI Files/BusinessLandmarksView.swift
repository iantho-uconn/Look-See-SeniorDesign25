...
struct BusinessLandmarksView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $bulkPromotionSelection) { selection in
            BusinessBulkPromotionEditor(landmarks: selection.landmarks) { result in
                handleBulkPromotionResult(result)
            }
        }
        .sheet(item: $bulkDeleteSelection) { selection in
            BusinessBulkDeleteView(landmarks: selection.landmarks) { result in
                handleBulkDeleteResult(result)
            }
        }
        .sheet(item: $landmarkNeedingMedia) { landmark in
            NeedsMoreMediaSheet(
                landmark: landmark,
                onAddMedia: {
    ...
    private var displayedLandmarks: [BusinessLandmark] {
        ...
        guard !query.isEmpty else { return viewModel.landmarks }

        let matches: [(landmark: BusinessLandmark, priority: Int)] = viewModel.landmarks.compactMap { landmark in
            if landmark.label.localizedCaseInsensitiveContains(query) { return (landmark, 0) }
            if searchablePromotionTitles(for: landmark).contains(where: { $0.localizedCaseInsensitiveContains(query) }) { return (landmark, 1) }
            return nil
        }
    ...
    private var noSearchResultsView: some View {
        VStack(spacing: 10) {
            Image(systemName: "magnifyingglass").font(.system(size: 28, weight: .semibold)).foregroundStyle(.secondary)
            Text("No landmarks found").font(.system(size: 17, weight: .bold, design: .rounded)).foregroundStyle(.primary)
            Text("Try a landmark label or promotion title.").font(.system(size: 14, weight: .medium)).foregroundStyle(.secondary).multilineTextAlignment(.center)
        }
        .frame(maxWidth: .infinity).padding(.vertical, 28).padding(.horizontal, 20)
        .background(Color(uiColor: .secondarySystemGroupedBackground)).clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous)).padding(.horizontal)
    }
    ...
    private var loadingView: some View {
        VStack(spacing: 14) {
            ...
            Text("Loading your landmarks...").font(.system(size: 14, weight: .medium)).foregroundColor(.secondary)
        }
        .frame(maxWidth: .infinity).padding(.vertical, 40)
    }
    
    private func toggleSelection(for landmark: BusinessLandmark) {
        if selectedLandmarkIds.contains(landmark.landmarkId) { selectedLandmarkIds.remove(landmark.landmarkId) } else { selectedLandmarkIds.insert(landmark.landmarkId) }
    }
    private func selectVisibleLandmarks() { selectedLandmarkIds.formUnion(visibleLandmarkIds) }
    private func deselectVisibleLandmarks() { selectedLandmarkIds.subtract(visibleLandmarkIds) }
    private func endSelectionMode() { isSelectionMode = false; selectedLandmarkIds.removeAll() }
    private func beginBulkPromotion() { bulkPromotionSelection = BulkLandmarkSelection(landmarks: selectedLandmarks) }
    private func beginBulkDelete() { bulkDeleteSelection = BulkLandmarkSelection(landmarks: selectedLandmarks) }

    @MainActor private func handleBulkPromotionResult(_ result: BusinessBulkPromotionResult) {
        ...
        for landmarkId in result.successfulLandmarkIds {
            ...
            if !titles.contains(where: { $0.caseInsensitiveCompare(result.promotionName) == .orderedSame }) { titles.append(result.promotionName) }
            promotionTitlesByLandmarkId[landmarkId] = titles
        }
        let failedIds = Set(result.failedLandmarks.map(\.landmarkId))
        selectedLandmarkIds = failedIds
        if failedIds.isEmpty { isSelectionMode = false }
    }

    @MainActor private func handleBulkDeleteResult(_ result: BusinessBulkDeleteResult) {
        viewModel.removeLandmarks(landmarkIds: result.successfulLandmarkIds)
        for landmarkId in result.successfulLandmarkIds { promotionTitlesByLandmarkId.removeValue(forKey: landmarkId) }
        let failedIds = Set(result.failedLandmarks.map(\.landmarkId))
        selectedLandmarkIds = failedIds
        if failedIds.isEmpty { isSelectionMode = false }
    }
    ...
    @MainActor private func loadPromotionSearchIndex(forceReload: Bool = false) async {
        let currentLandmarks = viewModel.landmarks
        let validLandmarkIds = Set(currentLandmarks.map(\.landmarkId))
        promotionTitlesByLandmarkId = promotionTitlesByLandmarkId.filter { validLandmarkIds.contains($0.key) }
        guard !currentLandmarks.isEmpty else { isIndexingPromotionTitles = false; return }
        let landmarksToLoad = forceReload ? currentLandmarks : currentLandmarks.filter { promotionTitlesByLandmarkId[$0.landmarkId] == nil }
        guard !landmarksToLoad.isEmpty else { isIndexingPromotionTitles = false; return }
        ...
        for landmark in landmarksToLoad {
            ...
            do {
                ...
                let titles = response.items.map { $0.name.trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty }
    ...
    private func searchablePromotionTitles(for landmark: BusinessLandmark) -> [String] {
        ...
        if let legacyPromotion = landmark.promotion?.trimmingCharacters(in: .whitespacesAndNewlines), !legacyPromotion.isEmpty, !titles.contains(where: { $0.caseInsensitiveCompare(legacyPromotion) == .orderedSame }) {
    ...
    private func matchedPromotionTitle(for landmark: BusinessLandmark) -> String? {
        let query = cleanedSearchText
        guard !query.isEmpty, !landmark.label.localizedCaseInsensitiveContains(query) else { return nil }
    ...
    private var syncBannerRow: some View {
        ...
        .padding(20).background(isUploading ? primaryColor.opacity(0.05) : Color.clear)
    }
        ...
        Button {
            ...
            HStack(spacing: 16) {
                ZStack {
                    ...
                    Image(systemName: item.isVideo ? "video.fill" : "photo.fill").font(.system(size: 18)).foregroundColor(.primary)
                }.frame(width: 48, height: 48).clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))

                VStack(alignment: .leading, spacing: 4) {
                    Text(item.title).font(.system(size: 16, weight: .bold, design: .rounded)).foregroundColor(.primary)
                    if isUploading {
                        VStack(alignment: .leading, spacing: 4) {
                ...
                if !isUploading {
                    Button {
                        ...
                        Image(systemName: "trash.fill").font(.system(size: 16, weight: .bold)).foregroundStyle(.red).frame(width: 36, height: 36).background(Color.red.opacity(0.1)).clipShape(Circle())
                    }
                }
            }
    ...
    var body: some View {
            ...
            HStack(alignment: .top, spacing: 14) {
                if isSelectionMode {
                    Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                        ...
                        .foregroundStyle(isSelected ? selectionColor : Color(uiColor: .tertiaryLabel))
                        .padding(.top, 1)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if let matchedPromotionTitle {
                        HStack(spacing: 6) {
                            ...
                            Text("Matched promotion: \(matchedPromotionTitle)").lineLimit(1)
                        }
            ...
            if needsMoreMedia {
                Button {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text("Not enough video data to train")
                            .font(.system(size: 13, weight: .bold, design: .rounded))
                        Spacer()
                        Text("Details").font(.system(size: 13, weight: .bold)).underline()
                    }
                    .padding(.horizontal, 20)
                    .padding(.vertical, 12)
        ...
        .background {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(isSelected ? selectionColor.opacity(0.10) : Color(uiColor: .secondarySystemGroupedBackground))
        }
        .overlay {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(
                    needsMoreMedia ? Color.red.opacity(0.6) : (isSelected ? selectionColor.opacity(0.60) : Color.clear),
                    lineWidth: 1.5
                )
        }
        .clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous))
        .shadow(color: needsMoreMedia ? Color.red.opacity(0.2) : .black.opacity(0.03), radius: 8, x: 0, y: 2)
        .animation(.easeOut(duration: 0.18), value: isSelected)
    }
}

// MARK: - The Action Required Bottom Sheet
struct NeedsMoreMediaSheet: View {
